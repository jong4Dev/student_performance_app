...
def load_data():
    ...
    df_combined["lulus"] = df_combined["G3"].apply(
...
if analysis_section == "Data Overview":
    ...
    # New section to show original datasets
    st.subheader("Original Datasets Preview")

    tab1, tab2 = st.tabs(["Math Dataset", "Portuguese Dataset"])

    with tab1:
        st.write("Math Dataset (First 10 rows)")
        df_math = pd.read_csv("data/student-mat.csv", sep=";")
        st.dataframe(df_math.head(10))

        st.write("Math Dataset Statistics")
        st.dataframe(df_math.describe())

        csv_math = df_math.to_csv(index=False).encode("utf-8")
        st.download_button(
            label="Download Math Dataset",
            data=csv_math,
            file_name="student_math.csv",
            mime="text/csv",
        )

    with tab2:
        st.write("Portuguese Dataset (First 10 rows)")
        df_por = pd.read_csv("data/student-por.csv", sep=";")
        st.dataframe(df_por.head(10))

        st.write("Portuguese Dataset Statistics")
        st.dataframe(df_por.describe())

        csv_por = df_por.to_csv(index=False).encode("utf-8")
        st.download_button(
            label="Download Portuguese Dataset",
            data=csv_por,
            file_name="student_por.csv",
            mime="text/csv",
        )
    ...
    with st.form("prediction_form"):
        ...
        with col1:
            ...
            address = st.selectbox(
                "Address", list(CATEGORICAL_MAPPINGS["address"].keys())
            )
            famsize = st.selectbox(
                "Family Size", list(CATEGORICAL_MAPPINGS["famsize"].keys())
            )
            Pstatus = st.selectbox(
                "Parents' Status", list(CATEGORICAL_MAPPINGS["Pstatus"].keys())
            )
        ...
        with col2:
            ...
            Medu = st.selectbox(
                "Mother's Education",
                [0, 1, 2, 3, 4],
                format_func=lambda x: ["None", "Primary", "Middle", "High", "Higher"][
                    x
                ],
            )
            Fedu = st.selectbox(
            ...
            Mjob = st.selectbox(
                "Mother's Job", list(CATEGORICAL_MAPPINGS["Mjob"].keys())
            )
            Fjob = st.selectbox(
                "Father's Job", list(CATEGORICAL_MAPPINGS["Fjob"].keys())
            )
            reason = st.selectbox(
                "School Choice Reason", list(CATEGORICAL_MAPPINGS["reason"].keys())
            )
            guardian = st.selectbox(
                "Guardian", list(CATEGORICAL_MAPPINGS["guardian"].keys())
            )
        ...
        with col3:
            ...
            traveltime = st.selectbox(
                "Travel Time",
                [1, 2, 3, 4],
                format_func=lambda x: ["<15min", "15-30min", "30-60min", ">60min"][
                    x - 1
                ],
            )
            ...
            schoolsup = st.selectbox(
                "School Support", list(CATEGORICAL_MAPPINGS["schoolsup"].keys())
            )
            famsup = st.selectbox(
                "Family Support", list(CATEGORICAL_MAPPINGS["famsup"].keys())
            )
            paid = st.selectbox(
                "Paid Classes", list(CATEGORICAL_MAPPINGS["paid"].keys())
            )
        ...
        with col4:
            activities = st.selectbox(
                "Activities", list(CATEGORICAL_MAPPINGS["activities"].keys())
            )
            nursery = st.selectbox(
                "Attended Nursery", list(CATEGORICAL_MAPPINGS["nursery"].keys())
            )
        with col5:
            higher = st.selectbox(
                "Wants Higher Ed", list(CATEGORICAL_MAPPINGS["higher"].keys())
            )
            internet = st.selectbox(
                "Internet Access", list(CATEGORICAL_MAPPINGS["internet"].keys())
            )
        with col6:
            romantic = st.selectbox(
                "In Relationship", list(CATEGORICAL_MAPPINGS["romantic"].keys())
            )
            subject = st.selectbox(
                "Subject", list(CATEGORICAL_MAPPINGS["subject"].keys())
            )
    ...
    if submitted:
        ...
        with col_res2:
            ...
            bars = ax.barh(
                ["Pass", "Fail"], [pass_prob, fail_prob], color=["#4CAF50", "#F44336"]
            )
            ax.set_xlim(0, 100)
            ...
            pd.DataFrame(
                {
                    ...
                    "Actual Value": [
                        {
                            v: k for k, v in CATEGORICAL_MAPPINGS.get(col, {}).items()
                        }.get(val, val)
